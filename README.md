...
## GPU deployment (Docker Hub + GitHub Actions)

This repo includes a workflow at `.github/workflows/docker-amd64.yml` that builds and pushes a Linux `amd64` backend image.
...
Image tags pushed by the workflow:

- `<DOCKERHUB_USERNAME>/fertilizer-backend:latest`
- `<DOCKERHUB_USERNAME>/fertilizer-backend:<git-sha>`

The workflow builds with:

- `backend/Dockerfile`
- `TORCH_WHL_INDEX=https://download.pytorch.org/whl/cu121` (CUDA wheel)

### Server runbook (Ubuntu/Debian, NVIDIA GPU)
...
2. Upload model files to `/opt/fertilizer/models`:

```bash
mkdir -p /opt/fertilizer/models
# Required:
#   best_model.pth
#   regression_model.pkl
# Optional:
#   best_uncoated_model.pth
```

3. Run backend container with GPU:

```bash
docker pull <DOCKERHUB_USERNAME>/fertilizer-backend:latest
docker rm -f fertilizer-backend || true
docker run -d \
  --name fertilizer-backend \
  --restart unless-stopped \
  --gpus all \
  -p 9000:8000 \
  -v /opt/fertilizer/models:/app/backend/models:ro \
  <DOCKERHUB_USERNAME>/fertilizer-backend:latest
```

4. Verify health and CUDA device:
